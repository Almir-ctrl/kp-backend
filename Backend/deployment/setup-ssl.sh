...
# SSL Certificate Setup Script for AI Music Separator Backend
# This script creates self-signed certificates with proper Subject Alternative Names (SAN)
...
# Generate improved self-signed certificate with SAN extensions
docker run --rm -v $(pwd)/nginx/ssl:/ssl alpine/openssl req -x509 -newkey rsa:4096 -sha256 -days 365 -nodes \
  -keyout /ssl/key.pem \
  -out /ssl/cert.pem \
  -subj "/C=US/ST=Development/L=Local/O=AI Music Separator/OU=Backend/CN=localhost" \
  -addext "subjectAltName=DNS:localhost,DNS:*.localhost,IP:127.0.0.1"

echo "✅ SSL certificates generated successfully!"
echo "📋 Certificate details:"
docker run --rm -v $(pwd)/nginx/ssl:/ssl alpine/openssl x509 -in /ssl/cert.pem -text -noout | head -20
